...
document.querySelectorAll('.paths > a').forEach((link,index)=>{
  ...
  const finish=()=>{
    ...
    timer=setTimeout(()=>{if(!destination.startsWith('#')){window.location.assign(destination);return;}const target=document.querySelector(destination);if(target){history.replaceState(null,'',destination);target.scrollIntoView({behavior:matchMedia('(prefers-reduced-motion: reduce)').matches?'instant':'smooth',block:'start'});target.setAttribute('tabindex','-1');target.classList.add('slider-arrival-focus');target.addEventListener('blur',()=>target.classList.remove('slider-arrival-focus'),{once:true});target.focus({preventScroll:true});}timer=setTimeout(reset,1200);},550);
  };
